...
export const SearchBar = ({ setResults, verticalKey }) => {
  ...
  const searchLoading =
    useSearchState((state) => state.vertical.searchLoading) || false;
  ...
  const allFacetOptions =
    facet?.options.map((option) => console.log(JSON.stringify(option))) || [];

  !searchLoading && console.log(JSON.stringify(allFacetOptions));
  useEffect(() => {
    if (facetWasPressed) {
